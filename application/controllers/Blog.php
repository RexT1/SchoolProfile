...
class Blog extends CI_Controller
{
	...
	function kategori()
	{
		...
		if ($query->num_rows() > 0) {
			...
			echo $this->session->set_flashdata('msg', '<div class="alert alert-danger">Tidak Ada artikel untuk kategori tersebut <b>' . $kategori . '</b></div>');
			redirect('artikel');
		}
	}
	...
	function search()
	{
		...
		if ($query->num_rows() > 0) {
			...
			echo $this->session->set_flashdata('msg', '<div class="alert alert-danger">Tidak dapat menemukan artikel dengan kata kunci tersebut <b>' . $keyword . '</b></div>');
			redirect('artikel');
		}
	}
